/**
 * Fantasy NBA Frontend App - Production Version with ET timezone
 */
...
class FantasyNBAApp {
    ...
    render() {
        ...
        app.innerHTML = `
            <div class="container">
                ...
                <div class="tabs">
                    ${weeks.map((w, i) => `<button class="tab-btn ${i === this.currentWeek ? 'active' : ''}" onclick="app.switchWeek(${i})" title="${w.fullLabel}">${w.label}</button>`).join('')}
                </div>
                <div id="week-content">${this.renderWeek(weeks[this.currentWeek])}</div>
            </div>
        `;
    }
    ...
    generateWeeks() {
        const weeks = [];
        // NBA 2025-26 season starts October 22, 2025
        const seasonStart = new Date('2025-10-22');

        // Adjust to Monday of season start week
        const day = seasonStart.getDay();
        const monday = new Date(seasonStart);
        monday.setDate(seasonStart.getDate() - day + (day === 0 ? -6 : 1));

        // Generate all weeks until end of April (approximately 27 weeks)
        const numWeeks = 27;
        ...
        for (let i = 0; i < numWeeks; i++) {
            ...
            weeks.push({
                label: `W${i + 1} ${this.formatDate(start)}-${this.formatDate(end)}`,
                fullLabel: `${this.formatDate(start)} - ${this.formatDate(end)}`,
                start, end
            });
        }

        // Find current week in ET
        const now = new Date();
        const etNow = new Date(now.toLocaleString('en-US', { timeZone: 'America/New_York' }));
        const currentWeekIndex = weeks.findIndex(w => etNow >= w.start && etNow <= w.end);

        // Set current week, default to first week if not found
        this.currentWeek = currentWeekIndex >= 0 ? currentWeekIndex : 0;
    ...
    renderWeek(week) {
        ...
        return `
            <div class="week-content">
                <h2 style="text-align:center;margin-bottom:20px">${week.fullLabel}</h2>
    ...
    getLastUpdate() {
        ...
        const date = new Date(Math.max(...updates.map(d => new Date(d))));
        // Display in ET timezone
